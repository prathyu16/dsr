...
void
DSRAgent::coordBcastPkt(Packet *packet)
  /* handle packets with a MAC destination address of this host, or
     the MAC broadcast addr */
{
  SRPacket p;
  ...
  iph->dport() = RT_PORT;
  //iph->saddr() = Address::instance().create_ipaddr(net_id.getNSAddr_(),RT_PORT);
  iph->sport() = RT_PORT;
  iph->ttl() = 30;

  cmh->ptype() = PT_DSR;
  cmh->size() = size_ + IP_HDR_LEN;
...
/* void modified_routereq(mypacket* my_packet,Packet* packet)
{
	if(dir==1 &&(node.X_ > mypacket->X && node.Y_ > mypacket->Y))
		handleRouteRequest(SRPacket &p)
	else if(dir==2 &&(node.X_ < mypacket->X && node.Y_ > mypacket->Y))
		handleRouteRequest(SRPacket &p)
	else if(dir==3 &&(node.X_ < mypacket->X && node.Y_ < mypacket->Y))
		handleRouteRequest(SRPacket &p)
	else if(dir==4 &&(node.X_ > mypacket->X && node.Y_ < mypacket->Y))
		handleRouteRequest(SRPacket &p)
} */


void DSRAgent :: flood_compare(Packet *packet)
{
   SRPacket p;
   hdr_sr *srh =  hdr_sr::access(p.pkt);
...
void DSRAgent ::compare(Packet *packet)
{
   SRPacket p;
   hdr_sr *srh =  hdr_sr::access(p.pkt);
   

   if ( node_->X() > srh->sr_coord_.x && node_->Y() > srh->sr_coord_.y )
        srh->sr_coord_.first++;
   else if ( node_->X() < srh->sr_coord_.x && node_->Y() > srh->sr_coord_.y )
        srh->sr_coord_.second++;
   else if ( node_->X()< srh->sr_coord_.x && node_->Y() < srh->sr_coord_.y )
        srh->sr_coord_.third++;
   else if ( node_->X() > srh->sr_coord_.x && node_->Y() < srh->sr_coord_.y )
        srh->sr_coord_.four++;
